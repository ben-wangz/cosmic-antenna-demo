...
public class ChannelDataParser implements FlatMapFunction<AntennaData, ChannelAntennaData> {
  ...
  @Builder
  @Jacksonized
  public ChannelDataParser(@NonNull Integer timeSampleUnitSize,
  ...
  public void flatMap(AntennaData antennaData, Collector<ChannelAntennaData> collector)
      throws Exception {
    LOGGER.debug("before channel data parse, original info [length:{}, header:{}]",
    ...
    for (int channelId = 0; channelId < channelSize; channelId++) {
      ...
      LOGGER.debug("during parse channel data -> channelId {}, extract index: [{},{})", channelId, startIndex, endIndex);
      ChannelAntennaData channelAntennaData = ChannelAntennaData.builder()
              .channelId(channelId)
              ...
              .build();
      LOGGER.debug("after channel data  parse, got object -> {}, array info[length:{}, header:{}]",
              channelAntennaData,
