...
import com.example.flink.SensorApp;
...
public class MultiplyWithCoefficient extends ProcessWindowFunction<SampleData, BeamData, Integer, TimeWindow> {
...
//        String tempFilePath = configuration.get(CosmicAntennaConf.COEFFICIENT_DATA_PATH);
        ...
        List<CoefficientData> coefficientDataList =
                OBJECT_MAPPER.readValue(Paths.get(tempFilePath).toFile(), new TypeReference<>() {
                });
    ...
    public void process(
            ...
            Collector<BeamData> collector) throws Exception {
        ...
        try (Mat dataRealMat = new Mat(antennaSize, timeSampleUnitSize, CV_64FC1);
             ...
             Mat coefficientRealMat = imdecode(new Mat(coefficientData.getRealArray()), IMREAD_UNCHANGED);
             Mat coefficientImaginaryMat = imdecode(new Mat(coefficientData.getImaginaryArray()), IMREAD_UNCHANGED)
        ) {
