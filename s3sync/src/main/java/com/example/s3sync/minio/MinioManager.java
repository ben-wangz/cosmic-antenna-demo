...
public class MinioManager {
  ...
  public static String completeDirectoryPath(String directoryPath) {
    return StringUtils.endsWith(directoryPath, "/")
        ? directoryPath
        : String.format("%s/", directoryPath);
  }
  ...
  public Iterable<MinioFile> objectList(String bucketName, String prefix, boolean recursive) {
    Iterable<Result<Item>> itemResultIterable =
    ...
    return () ->
        new Iterator<>() {
          ...
          public MinioFile next() {
            try {
              Item item = itemResultIterator.next().get();
              if (null == item) {
                return null;
              }
  ...
  public void objectRemove(String bucketName, String objectKey, boolean ignoreNotExists)
      throws MinioException {
  ...
  public InputStream objectGetFromOffset(String bucketName, String objectKey, long offset)
      throws MinioException {
    if (!objectExists(bucketName, objectKey)) {
      throw new MinioException(objectNotExists(objectKey));
    }
    try {
      return minioClient.getObject(
          GetObjectArgs.builder().bucket(bucketName).object(objectKey).offset(offset).build());
    } catch (ErrorResponseException
        | InsufficientDataException
        | InternalException
        | InvalidKeyException
  ...
  public void objectCopy(
      String bucketName, String sourceObjectKey, String targetObjectKey, boolean override)
      throws MinioException {
  ...
  public void directoryCreate(
      String bucketName, String directoryPath, boolean createParent, boolean ignoreExists)
      throws MinioException {
    ...
    objectUpload(new ByteArrayInputStream(new byte[] {}), bucketName, completeDirectoryPath);
  }

  public void directoryRemove(
      String bucketName, String directoryPath, boolean recursive, boolean ignoreNotExists)
      throws MinioException {
    String completeDirectoryPath = completeDirectoryPath(directoryPath);
    ...
    if (!recursive) {
      if (directoryIsEmpty(bucketName, completeDirectoryPath)) {
        throw new MinioException(
            String.format("directoryPath is not empty(%s)", completeDirectoryPath));
      }
      objectRemove(bucketName, completeDirectoryPath, true);
      return;
    }
    List<MinioFile> minioFileList =
        StreamSupport.stream(
                objectList(bucketName, completeDirectoryPath, true).spliterator(), false)
            .collect(Collectors.toList());
    Iterable<Result<DeleteError>> deleteErrors =
        minioClient.removeObjects(
            RemoveObjectsArgs.builder()
                .bucket(bucketName)
                .objects(
                    Stream.concat(
                            minioFileList.stream().map(MinioFile::getObjectKey),
                            Stream.of(completeDirectoryPath))
                        .map(DeleteObject::new)
                        .collect(Collectors.toList()))
                .build());
    List<DeleteError> deleteErrorList =
        StreamSupport.stream(deleteErrors.spliterator(), false)
            .map(
                deleteErrorResult -> {
                  try {
  ...
  public boolean directoryExists(String bucketName, String directoryPath) throws MinioException {
    ...
    return objectList(bucketName, completeDirectoryPath, true).iterator().hasNext();
  }

  public void directoryCopy(
      String bucketName, String sourceDirectoryPath, String targetDirectoryPath, boolean override)
      throws MinioException {
    ...
    List<MinioFile> minioFileList =
        StreamSupport.stream(minioFiles.spliterator(), false).collect(Collectors.toList());
    for (MinioFile minioFile : minioFileList) {
      String sourceObjectKey = minioFile.getObjectKey();
      String suffix =
          StringUtils.removeStart(sourceObjectKey, source)
              .replaceAll("//+$", "/")
              .replaceAll("^/+", "");
  ...
  private Optional<String> parentPathOptional(String path) {
    ...
    String pathWithLastSlashPartRemoved =
  ...
  private boolean directoryIsEmpty(String bucketName, String completeDirectoryPath) {
    ...
    return minioFiles.iterator().hasNext();
  }
}
